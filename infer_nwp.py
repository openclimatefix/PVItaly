...
def infer(  # type: ignore[no-any-unimported]
    ...
    Returns:
        ...
        pv_t0_idx: int
    """
    pv_data_pipeline = nwp_pv_datapipe(config, tag="test")
...
num_batches = 4 * 10*31 * 24 * 4 / 32
# 4 systems, 10 months, 24*4 15 min periods, / 32 examples in each batch

def main(
    config: Path = Path("inference/infer.yaml"),
    module: str = "experiments.e004.experiment_004",
    ckpt_dir: Path = Path("./ckpt/"),
    batches: int = int(num_batches),
    # batches = 10,
